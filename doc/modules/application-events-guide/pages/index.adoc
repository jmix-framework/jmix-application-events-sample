...
There are various events you can subscribe to that handle the lifecycle of a controller, such as `InitEvent`, `BeforeCloseEvent`, and `BeforeSaveEvent`. The data components of the controllers also offer events like `ItemChangeEvent` and `CollectionChangeEvent`. Additionally, UI components themselves trigger events for changes in their state, such as `EnterPressEvent` and `TypedValueChangeEvent`.
